...
HRESULT EF_Slash::init(POINT position, float angle, float speed, float damage)
{
	_x = position.x;
	_y = position.y;

	_img = new image;
	_img->init("image/effect/slash.bmp", 302, 258, 2, 2, true, RGB(255, 0, 255));
...
void EF_Slash::release()
{
	_img->release();
	SAFE_DELETE(_img);
}

void EF_Slash::update()
{
	//move
