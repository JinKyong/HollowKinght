...
HRESULT EF_DownSlash::init(POINT position, float angle, float speed, float damage)
{
	_x = position.x;
	_y = position.y;

	_img = new image;
	_img->init("image/effect/downslash.bmp", 258, 302, 2, 2, true, RGB(255, 0, 255));
...
void EF_DownSlash::release()
{
	_img->release();
	SAFE_DELETE(_img);
}

void EF_DownSlash::update()
{
	//move
...
void EF_DownSlash::controlFrame()
{
